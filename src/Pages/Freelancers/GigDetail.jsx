...
import { Clock, Globe, Star, Edit2, ArrowLeft } from 'lucide-react'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              {gigData.images.map((image, index) => (
                <SwiperSlide key={index}>
                  <div className="relative h-full w-full rounded-md overflow-hidden cursor-pointer">
                    <img
                      src={image}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {gigData.relatedServices.map((service) => (
                  <Card key={service.id} className="overflow-hidden">
                    ...
                    <CardContent className="p-4">
                      ...
                      <p className="text-sm line-clamp-2">{service.title}</p>
                      <p className="text-sm font-semibold mt-2">Starting at ${service.price}</p>
                    </CardContent>
                  </Card>
                ))}
              </div>
            ...
            <Card className="sticky top-24">
              <CardContent className="p-6">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-2xl font-bold">${gigData.price.toFixed(2)}</span>
                </div>
                <div className="space-y-4 mb-6">
                  <div className="flex items-center gap-2">
                    <Clock className="h-4 w-4" />
                    <span>{gigData.deliveryTime}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Globe className="h-4 w-4" />
                    <span>{gigData.revisions}</span>
                  </div>
                </div>
                <Button className="w-full" size="lg" disabled>
                  Buy Now ${gigData.price.toFixed(2)}
                </Button>
